...
/**
 * @summary Send an email summary of the tattoo consultation conversation
 */
export const getEmailConversationSummaryUrl = (id: number) => {
  return `/api/openai/conversations/${id}/email-summary`;
};

export const emailConversationSummary = async (
  id: number,
  emailSummaryBody: EmailSummaryBody,
  options?: RequestInit,
): Promise<EmailSummaryResponse> => {
  return customFetch<EmailSummaryResponse>(getEmailConversationSummaryUrl(id), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(emailSummaryBody),
  });
};

export const getEmailConversationSummaryMutationOptions = <
  TError = ErrorType<OpenaiError>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof emailConversationSummary>>,
    TError,
    { id: number; data: BodyType<EmailSummaryBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  ...
  TContext
> => {
  const mutationKey = ["emailConversationSummary"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return emailConversationSummary(id, data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type EmailConversationSummaryMutationResult = NonNullable<
  Awaited<ReturnType<typeof emailConversationSummary>>
>;
export type EmailConversationSummaryMutationBody = BodyType<EmailSummaryBody>;
export type EmailConversationSummaryMutationError = ErrorType<OpenaiError>;

/**
 * @summary Send an email summary of the tattoo consultation conversation
 */
export const useEmailConversationSummary = <
  TError = ErrorType<OpenaiError>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getEmailConversationSummaryMutationOptions(options));
};
