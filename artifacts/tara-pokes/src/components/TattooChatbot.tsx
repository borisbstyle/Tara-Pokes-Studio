...
export default function TattooChatbot() {
  ...
  const ensureConversation = async (): Promise<number> => {
    ...
    const conv = await createConversation.mutateAsync({
      data: { title: "Tattoo Consultation" },
    });
  ...
  const handleSendEmail = async () => {
    ...
    setEmailError("");
    try {
      const result = await sendEmailSummary.mutateAsync({
        id: conversationId,
        data: { recipientEmail: email.trim() },
      });
      if (result.success) {
        setEmailSent(true);
      } else {
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {hasEnoughConversation && !emailSent && (
              <div className="px-4 pb-2 flex-shrink-0">
                {!showEmailForm ? (
                  ...
                  >
                    <Mail size={12} />
                    Email this consultation summary
                  </button>
                ) : (
            ...
            {emailSent && (
              <div
                className="mx-4 mb-2 flex items-center gap-2 px-3 py-2 text-xs"
                style={{
                  background: "hsl(var(--card))",
                  border: "1px solid hsl(var(--border))",
                  borderRadius: "0.5rem",
              ...
              >
                ...
                Summary sent. Tara will be in touch soon.
              </div>
            )}
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-4 md:right-8 z-50 flex items-center gap-2.5 px-4 py-3 rounded-full shadow-2xl"
        style={{
          background: "hsl(var(--primary))",
          color: "hsl(var(--primary-foreground))",
        }}
